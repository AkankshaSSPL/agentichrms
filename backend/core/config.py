...
class Settings(BaseSettings):
    ...
    # ── Database ───────────────────────────────────────────────────────────────
    # REQUIRED — no default. Credentials must never live in code.
    # Set DATABASE_URL in .env (see .env.example). App refuses to boot if missing.
    DATABASE_URL: str

    # ── Security & JWT ─────────────────────────────────────────────────────────
    # REQUIRED — no default. A known/default signing key makes every JWT
    # forgeable. Generate one with:
    #   python -c "import secrets; print(secrets.token_urlsafe(48))"
    # and set JWT_SECRET in .env.
    JWT_SECRET: str
    ALGORITHM: str = "HS256"
