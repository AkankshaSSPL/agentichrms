...
def _do_register(payload: EmployeeRegisterRequest, db: Session):
    ...
    pin_record = PINVerification(
        employee_id=new_employee.id,
        pin_hash=get_password_hash(pin),  # never store the raw PIN
    ...
    return {
        "message": "Registration successful!" if sms_result["success"]
                   else "Registered, but the SMS could not be sent. Use 'Request PIN' to retry, or contact HR.",
        "employee_id": new_employee.id,
        ...
        # NOTE: the PIN is never returned in the response (RULES R18). If SMS
        # delivery fails, the user re-requests a PIN rather than seeing it here.
    }