...
def verify_and_change_pin(payload: VerifyAndChangePinRequest, db: Session = Depends(get_db)):
    ...
    # Set new PIN — store ONLY the hash. Never persist the plaintext PIN.
    # (Removed `emp.permanent_pin = payload.new_pin`; that column is write-only
    #  and read nowhere — see CLEANUP_LOG. It should be dropped in a migration.)
