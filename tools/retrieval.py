...
class SourceTrackedRetriever:
    ...
    def extract_policy_type(self, query: str) -> Optional[str]:
        """
        Extract a policy-type signal from the query WITHOUT hardcoded keywords.

        Instead of mapping queries to a fixed list of 8 policy buckets, we extract
        the meaningful content words from the query itself and use them directly as
        the policy-type signal for matching against each chunk's stored `section`
        and `policy_type` metadata fields.

        This means ANY term — "covenants not to compete", "return of materials",
        "indemnification", "probation period", "intellectual property" — is handled
        automatically because it comes directly from what the user typed, not from
        a predefined list that someone had to maintain.

        Returns a normalised string of content words from the query, or None if
        the query is too short to produce a meaningful signal.
        """
        # Strip common question words and stopwords that add no signal
        STOPWORDS = {
            "what", "is", "are", "the", "a", "an", "about", "for", "of",
            "in", "on", "at", "to", "and", "or", "with", "how", "does",
            "do", "can", "will", "has", "have", "me", "my", "our", "your",
            "tell", "explain", "describe", "show", "give", "get", "find",
            "please", "policy", "rule", "rules", "guidelines", "procedure",
            "company", "hr", "employee", "employees", "according",
        }

        words = re.findall(r"\b[a-z][a-z\'\-]{2,}\b", query.lower())
        content_words = [w for w in words if w not in STOPWORDS]

        if not content_words:
            return None

        # Return the most meaningful phrase (up to 4 words) as the policy signal.
        # This is used downstream only for a small ranking bonus — it does not
        # gate retrieval. If it produces no match, retrieval still works via
    ...
    def rerank_chunks(
        ...
        FIX: Score ALL chunks before applying the top_k cut, then normalise
        all scores, then sort. Previously only the top_k chunks received a
        relevance_score — chunks outside top_k had no score, making the
        cosine backstop check in verification meaningless for them.
        """
        if not chunks:
            return []
        ...
        # Assign raw score to every chunk before any cut
        for chunk, score in zip(chunks, scores):
            chunk["relevance_score"] = float(score)

        # Normalise all scores together so sigmoid is calibrated on full pool
        chunks = self._normalize_scores(chunks)

        # Sort by normalised score and return top_k
    ...
    def filter_by_policy_type(
        ...
        """
        Tag chunks with policy_match = True/False WITHOUT reordering.

        policy_type is now content words extracted from the user query
        (e.g. "covenants compete" from "what is covenants not to compete").
        We check whether any of those words appear in the chunk's stored
        section header or policy_type metadata — both set from the document's
        own headings at ingest time, never from a hardcoded list.

        This means section="Covenants Not To Compete" matches the query
        "covenants not to compete" automatically, with no keyword list needed.

        The flag is only a small +0.1 ranking bonus. It never gates retrieval —
        embedding similarity always runs regardless.
        """
        for chunk in chunks:
            chunk.setdefault("policy_match", False)

        if not policy_type or not chunks:
            return chunks

        query_words = [w for w in policy_type.lower().split() if len(w) >= 3]
        if not query_words:
            return chunks
        ...
        for chunk in chunks:
            ...
            content_head = chunk.get("content", "")[:300].lower()

            chunk["policy_match"] = any(
                w in section or w in chunk_policy or w in content_head
    ...
    def verify_source_match(self, query: str, chunk_content: str) -> float:
        ...
        FIX: Use 3-char minimum (was 4) so short meaningful terms like
        WFH, NDA, act, law contribute to scoring instead of being ignored.
        """
        query_words = set(re.findall(r'\b\w{3,}\b', query.lower()))
        content_words = set(re.findall(r'\b\w{3,}\b', chunk_content.lower()))
    ...
    def retrieve_with_sources(
        self, query: str, k: int = 30, final_k: int = 7
    ) -> List[Dict[str, Any]]:
        """Retrieve with reranking and verification.

        FIX: Default k raised from 15 to 30 so filter_by_policy_type and the
        reranker have a larger candidate pool. With only 15 chunks, any tagging
        had an outsized effect on which chunks the reranker evaluated.
        """
        policy_type = self.extract_policy_type(query)
        if policy_type:
            logger.debug("Query content words for section matching: %s", policy_type)
        ...
        # Tag chunks with policy_match using query content words (no hardcoded list)
        chunks = self.filter_by_policy_type(chunks, policy_type)

        # Rerank ALL chunks — no pre-cut. top_k=len ensures every chunk is scored
        # before the verification threshold decides what to keep.
        reranked_chunks = self.rerank_chunks(query, chunks, top_k=len(chunks))

        verified_sources = []
        for chunk in reranked_chunks:
            v_score = self.verify_source_match(query, chunk["content"])
            chunk["verification_score"] = v_score
            # FIX: Threshold lowered 0.1 → 0.05. After sigmoid normalisation real
            # scores cluster in 0.3-0.7; 0.1 silently dropped valid chunks.
            # Cosine backstop: keep any chunk ChromaDB rated relevant (score > 0.3).
            if (chunk["relevance_score"] > 0.05
                    or chunk.get("policy_match")
                    or chunk.get("score", 0) > 0.3):
        ...
        verified_sources.sort(
            key=lambda x: (
                ...
                # Small 0.1 bonus for section-header match — never overrides
                # the reranker's relevance judgement
                + (0.1 if x.get("policy_match") else 0)
            ),
            reverse=True,
        )
...
def search_policies(query: str) -> Dict[str, Any]:
    ...
    """
    chunk_sources = _retriever.retrieve_with_sources(query, k=30, final_k=10)
