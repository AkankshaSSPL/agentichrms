...
def search_policies(query: str) -> Dict[str, Any]:
    ...
    """
    # Retrieve more sources (increase final_k from 7 to 10)
    ...
    for filename, group in other_groups.items():
        ...
        # Merge contiguous chunks into segments
        refined_segments = []
        if not sorted_group:
            continue
        ...
        for g in sorted_group[1:]:
            # Allow gap of up to 2 lines
            gap = g.get("start_line", 0) - current_sub[-1].get("end_line", 0)
            total_l = g.get("end_line", 0) - current_sub[0].get("start_line", 0)

            if gap <= 2 and total_l <= 50:   # merge if gap small and total lines ≤50
        ...
        # Create one single source card for this file
    ...
    # Deduplicate sources by filename and section (just in case)
    seen = set()
    deduped_sources = []
    ...
    # Build answer context for the LLM using ALL segments from each source
    results = []
    # Show up to 8 sources in the answer (adjust as needed)
    for i, src in enumerate(deduped_sources[:8], 1):
        full_text = ""

        if "full_content" in src and src["full_content"]:
            # PDF: combine all pages in the range (start_line to end_line)
            ...
            # Other docs: combine all segments
        ...
        # Trim if too long (5000 characters per source is a safe limit)
        if len(full_text) > 5000:
            full_text = full_text[:5000] + "... [truncated]"

        # Skip if completely empty (optional)
        if not full_text.strip():
            full_text = "[No text available for this source]"

        results.append(
            f"RESULT {i} [Source: {src['source_file']} | Section: {src['section']} | Lines: {src['start_line']}-{src['end_line']}]\n"
            f"{full_text}"
        )
