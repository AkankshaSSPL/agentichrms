...
def get_onboarding_checklist(employee_name: str) -> str:
    ...
    # GAP-024 FIX: Use context manager — connection is always closed even on exception.
    try:
...
def mark_task_complete(employee_name: str, task_name: str) -> str:
    ...
    try:
        with _get_conn() as conn:
            ...
            task = c.execute(
                "SELECT id, task_name FROM onboarding_tasks WHERE LOWER(task_name) LIKE ?",
                (f"%{task_name.lower()}%",),
            ).fetchone()
            if not task:
                return f"Task '{task_name}' not defined in system."
            task_id, canonical_name = task
            ...
            if c.rowcount == 0:
                return f"Task '{canonical_name}' is not assigned to {name}."
    ...
    return f"Task '{canonical_name}' marked as completed for {name}."
