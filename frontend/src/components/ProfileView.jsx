...
function ProfileEditChat({ token, onClose, onSaved }) {
    ...
    // Create DB session + send opening greeting on mount
    useEffect(() => {
        const init = async () => {
            setLoading(true)
            try {
                ...
                const res = await fetch(`${API}/api/chat/`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                    body: JSON.stringify({ message: 'Hello, I want to update my personal details.', session_id: sid }),
                })
                const data = await res.json()
                setMessages([{ role: 'assistant', content: data.answer }])
            } catch {
                setMessages([{ role: 'assistant', content: "What would you like to update? I can help with your address, emergency contact, gender, or date of birth." }])
            } finally {
                setLoading(false)
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                borderRadius: 24, width: '92%', maxWidth: 520, height: '78vh',
                ...
                <div style={{
                    ...
                    padding: '18px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-card)',
                }}>
                    <div>
                        <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)' }}>Update Profile</div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>Tell me what you want to update</div>
                    </div>
                    <button onClick={onClose} style={{ background: 'transparent', border: 'none', fontSize: 20, cursor: 'pointer', color: 'var(--text-muted)', padding: '4px 8px', borderRadius: 8 }}
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: 16, background: 'var(--bg-primary)' }}>
                    ...
                    {loading && (
                        <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                            <div style={{ padding: '10px 16px', borderRadius: '18px', background: 'var(--bg-card)', border: '1px solid var(--border)', display: 'flex', gap: 6, alignItems: 'center' }}>
                                {[0,1,2].map(i => <span key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--accent)', animation: `bounce 0.9s ease-in-out ${i*0.2}s infinite`, display: 'inline-block' }} />)}
                            </div>
                        </div>
                    )}
                    <div ref={endRef} />
...
export default function ProfileView({ employee, token, onBack, onSaved }) {
    ...
    const loadProfile = () => {
        setLoading(true)
        fetch(`${API}/api/onboarding-profile/me`, { headers: { Authorization: `Bearer ${token}` } })
            .then(r => r.json())
    ...
    return (
        <>
            ...
            <div style={{ width: '100%', height: '100%', overflowY: 'auto', overflowX: 'hidden', background: 'var(--bg-primary)', fontFamily: "'Sora',sans-serif", position: 'relative' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1, width: '100%', padding: '20px 20px 80px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
                        <button onClick={onBack} style={{ background: 'var(--accent-dim)', border: '1px solid var(--border)', borderRadius: 10, padding: '9px 18px', color: 'var(--accent)', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>← Back</button>
