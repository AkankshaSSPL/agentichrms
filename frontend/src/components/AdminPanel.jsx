        ...
        <div style={{ position: 'fixed', top: 24, right: 24, zIndex: 9999, display: 'flex', flexDirection: 'column', gap: 10, pointerEvents: 'none' }}>
            {alerts.map(a => (
                <div key={a.id} style={{
                    ...
                    minWidth: 280, maxWidth: 400, pointerEvents: 'all', animation: 'slideIn 0.25s ease',
                }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.70)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: 'var(--bg-card)', border: `1px solid ${s.border}`, borderRadius: 20, padding: '40px 36px', maxWidth: 420, width: '90%', boxShadow: '0 24px 64px rgba(0,0,0,0.4)', animation: 'popIn 0.22s ease', textAlign: 'center', position: 'relative' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', margin: '0 0 18px', fontSize: 13, lineHeight: 1.6 }}>
                    ...
                    <span style={{ color: s.color, fontWeight: 700, background: s.bg, padding: '2px 10px', borderRadius: 12, fontSize: 12, border: `1px solid ${s.border}` }}>{info.role}</span>
                </p>
                <p style={{ color: 'var(--text-muted)', fontSize: 12, margin: '0 0 24px' }}>
                    A notification email has been sent to <strong style={{ color: 'var(--text-secondary)' }}>{info.email}</strong>
                </p>
                <button onClick={onClose} style={{ background: s.bg, border: `1px solid ${s.border}`, color: s.color, padding: '10px 32px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Got it</button>
            </div>
        </div>
    )
}
...
function EmailSettingsTab({ token }) {
    ...
    const triggerLabel = {
        leave_approve: 'Leave Approved', leave_reject: 'Leave Rejected',
        role_change: 'Role Changed', test: 'Test', system: 'System',
    }
...
export default function AdminPanel({ token: tokenProp }) {
    ...
    const updateRole = async (employeeId, newRole, employeeName) => {
        ...
        try {
            const res = await fetch(`${API}/admin/role`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ employee_id: employeeId, role_name: newRole }) })
    ...
    const deleteEmployee = async (employeeId, employeeName) => {
        const ok = await confirm('Delete Employee', `Are you sure you want to permanently delete ${employeeName}? This action cannot be undone.`, 'Delete')
        if (!ok) return
        try {
            const res = await fetch(`${API}/admin/employees/${employeeId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
        ...
        <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100%', background: 'var(--bg-primary)', padding: '32px 40px', boxSizing: 'border-box' }}>
                ...
                <div style={{ marginBottom: 28 }}>
                    <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.3px' }}>Admin Dashboard</h1>
                    <p style={{ margin: '4px 0 0', color: 'var(--text-secondary)', fontSize: 13 }}>Manage roles, review leave requests, and configure system settings</p>
                </div>
                ...
                {/* Dashboard */}
                {activeTab === 'dashboard' && (
                    <DashboardMetricsSimple employees={employees} token={token} onNavigate={setActiveTab} />
                )}
                            ...
                            <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border)', overflow: 'hidden' }}>
                                <div style={{ overflowX: 'auto' }}>
                                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 640 }}>
                                        ...
                                        <tbody>
                                            {employees.length === 0 ? (
                                                <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>No employees found</td></tr>
                                            ) : employees.map((emp, i) => {
                                ...
                                <div style={{ padding: '10px 18px', borderTop: '1px solid var(--border)', color: 'var(--text-muted)', fontSize: 11 }}>
