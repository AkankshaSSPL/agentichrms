...
def split_markdown_by_headers(content: str, filename: str):
    ...
    def _detect_policy_type(text: str) -> str:
        """
        Use the document's own header text as the policy_type — no hardcoded
        keyword list needed.

        Previously this function matched header text against 8 fixed policy
        buckets ("leave", "moonlighting", etc.). Any section not in the list
        was tagged "General", which meant the policy_match ranking bonus never
        fired for NDA clauses, covenants, return of materials, etc.

        Now we simply normalise and store the header text itself as the
        policy_type. At query time, retrieval.py splits the user's query into
        content words and checks whether they appear in this stored header —
        so "covenants not to compete" matches section="Covenants Not To Compete"
        automatically, with no keyword list to maintain.
        """
