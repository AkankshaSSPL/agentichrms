...
def main():
    ...
    # Normalized leave_balances: one row per (employee, leave_type)
    ...
    # Normalized leave balances: one row per (employee, leave_type)
    leave_types = {
        "casual": 12, "sick": 10, "earned": 15, "maternity": 182, "paternity": 15
    }
    for emp in employees:
        for l_type, days in leave_types.items():
            c.execute("INSERT INTO leave_balances (employee_id, leave_type, days_remaining) VALUES (?, ?, ?)",
                      (emp[0], l_type, days))

    # Adjust balances for employees with existing leave history
    ...
    for emp_id, l_type, start, end, status, reason in leaves:
        c.execute("""INSERT INTO leaves (employee_id, leave_type, start_date, end_date, status, reason)
                     VALUES (?, ?, ?, ?, ?, ?)""", (emp_id, l_type, start, end, status, reason))
    ...
    for emp_id in [3, 16, 20]:
        for task in tasks:
            status = "Completed" if task[0] <= 4 and emp_id == 3 else "Pending"
            completed_at = "2024-01-25" if status == "Completed" else None
            c.execute("INSERT INTO employee_onboarding (employee_id,task_id,status,completed_at) VALUES (?,?,?,?)",
                     (emp_id, task[0], status, completed_at))

    conn.commit()
    conn.close()
    print(f"Database created at {DB_PATH}")
    print(f"  20 employees, {20 * len(leave_types)} leave balances, {len(leaves)} leaves, {len(tasks)} onboarding tasks")
