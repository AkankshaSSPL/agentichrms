...
with st.sidebar:
    ...
    uploaded = st.file_uploader("Upload docs", type=["pdf", "md", "csv", "xlsx", "txt"], accept_multiple_files=True, label_visibility="collapsed")
    if uploaded:
...
with col_chat:
    ...
    for msg in st.session_state.messages:
        if msg["role"] == "user":
            st.markdown(f'<div class="user-msg"><div class="u-avatar">👤</div><div class="u-bubble">{msg["content"]}</div></div>', unsafe_allow_html=True)
        else:
            answer_text = msg["content"]
            sources = msg.get("sources", [])
            
            # Formatting refs block
            refs_html = ""
            if sources:
                refs_html += '<div class="refs-block"><div class="refs-label">📎 Sources</div>'
                for src in sources:
                    filename = src.get('source_file', '')
                    file_exists = os.path.exists(os.path.join(DOCS_DIR, filename))
                    icon = "📄" if file_exists else "⚠️"
                    section = src.get('section', '')
                    refs_html += f'<div class="ref-row"><span>{icon}</span><span class="ref-file">{filename}</span><span class="ref-loc">— {section}</span></div>'
                refs_html += '</div>'
...
with col_preview:
    ...
    for idx, src in enumerate(latest_sources):
        filename = src.get("source_file", "")
        location = f"{src.get('section', 'General')} · Lines {src.get('start_line', 1)}–{src.get('end_line', 5)}"
        file_exists = os.path.exists(os.path.join(DOCS_DIR, filename))
        
        st.markdown(source_doc_card_html(filename, location, found=file_exists), unsafe_allow_html=True)
        
        if file_exists:
            is_expanded = (st.session_state.expanded_doc_idx == idx)
            btn_label = "Hide Viewer" if is_expanded else "Open Code Viewer"
...
if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
    ...
    # Greeting detection
    greeting_words = {"hi", "hello", "hey", "good morning", "good afternoon", "good evening", "greetings", "howdy", "hola", "namaste"}
    ...
    try:
        ...
        # Keep deduplicate but DO NOT filter existing! We want missing ones to show with error card.
